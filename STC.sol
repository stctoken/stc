...
/**
 * The base STC contract. Check method must be overrided.
 */
contract STC is SoftDestruct {
    ...
    /**
     * STC service admin account.
     */
    address private STCAccount;
    /**
    ...
    // ------------ CONSTRUCT -------------
    function STC(address _targetUser, address[] _recipients, uint8[] _percents)
    ...
    modifier onlyAdmin() {
        require(STCAccount == msg.sender);
        _;
    }

    modifier onlyTargetOrAdmin() {
        require(isTarget() || STCAccount == msg.sender);
        _;
    }
