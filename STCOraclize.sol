...
import "STC.sol";
import "STCOraclizeProxyI.sol";

contract STCOraclize is STC {
    STCOraclizeProxyI private proxy;
    /**
    ...
    /**
     * To inform STC system that it should repeat check immediately.
     */
    event NeedRepeatCheck(bool isAccident);
    /**
     * To inform STC system about latest oraclize price in wei.
     */
    event Price(uint);
    /**
    ...
    // ------------ CONSTRUCT -------------
    function STCOraclize(address _targetUser, address[] _recipients, uint8[] _percents, uint32 _noActivityPeriod, address _proxy)
             STC(_targetUser, _recipients, _percents) {
        noActivityPeriod = _noActivityPeriod;
        proxy = STCOraclizeProxyI(_proxy);
